...
EmbeddedOk --> |是| ReturnEmbedded["返回内嵌歌词"]
EmbeddedOk --> |否| FetchOnline
FetchOnline --> OnlineOk{"获取成功?"}
OnlineOk --> |是| Persist["必要时写入侧车/嵌入"]
Persist --> ReturnOnline["返回在线歌词"]
OnlineOk --> |否| ReturnNone["返回无歌词"]
...
## 结论
LyricsService 提供了从侧车、内嵌、网络多源获取歌词的能力，并通过稳定的解析与持久化策略保障用户体验。结合渲染侧的编辑、导入、保存与批量任务，形成完整的歌词管理闭环。建议在生产环境中配合合理的超时与节流策略，确保稳定性与性能平衡。
