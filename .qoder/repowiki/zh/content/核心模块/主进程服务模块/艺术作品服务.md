...
## 更新摘要
**变更内容**
- 增强了图像验证机制，使用魔数字节嗅探确保缓存的有效性
- 改进了缓存重建策略，自动修复历史污染的缓存条目
- 优化了最佳图片选择算法，增加了优先级排序和格式验证
- 新增了对过期系统缩略图的检测和重建功能
...
- 缓存工具集（artwork-cache.ts）：负责缩略图缓存写入、系统缩略图生成、缓存清理、格式推断、图像验证
...
**更新** 增强了缓存验证机制，自动检测和修复历史污染的缓存条目

```mermaid
flowchart TD
Start(["开始"]) --> GetRow["查询歌曲行<br/>Id/Path/ThumbnailPath"]
GetRow --> HasCached{"缓存存在且有效？"}
HasCached --> |是| VerifyCache["魔数字节验证<br/>isLikelyImage()"]
VerifyCache --> |有效| UseCached["返回缓存 URL<br/>source=cached"]
VerifyCache --> |无效| RebuildCache["重建缓存"]
...
- **新增** 图像验证：使用魔数字节嗅探验证缓存的有效性，防止历史污染

**更新** 新增了完整的图像验证和缓存重建机制
...
**更新** 改进了最佳图片选择算法，增加了魔数字节验证和优先级排序
...
- ArtworkService 依赖：
  - 数据库（读取歌曲行、更新缩略图路径）
  - 缓存工具集（写入缩略图缓存、生成系统缩略图、清理缓存、图像验证）
...
- **新增** 图像验证：魔数字节嗅探快速验证缓存有效性，避免加载无效文件
- **新增** 自动重建：检测过期缓存并自动重建，无需用户干预
- 建议优化：
  - 对高频访问的封面增加预加载队列
...
## 结论
SMPlayer 的艺术作品服务通过"缓存优先 + 元数据内嵌 + 系统缩略图"的三层来源策略，结合数据库持久化与文件系统写入，实现了稳定高效的封面管理。**最新更新增强了图像验证和缓存重建机制，通过魔数字节嗅探确保缓存的有效性，并自动修复历史污染的缓存条目，显著提升了系统的健壮性和用户体验。**前端通过批量请求与缓存策略显著降低 IPC 开销，配合错误回退与刷新机制提升用户体验。后续可在网络来源、内存缓存与预加载等方面进一步增强性能与扩展性。
...
- 魔数字节验证：通过检查文件头部的特定字节序列来确认文件是否为有效的图像格式
- 缓存重建：检测过期或损坏的缓存文件并自动重新生成
- 历史污染修复：自动识别并修复历史版本中产生的无效缓存条目
- 性能优化：魔数字节验证比完整的图像解码更快速，减少不必要的处理开销

**章节来源**
- [electron\services\artwork-cache.ts:16-52](file://electron/services/artwork-cache.ts#L16-L52)
- [electron\services\artwork-cache.ts:226-229](file://electron/services/artwork-cache.ts#L226-L229)
- [electron\services\artwork-service.ts:262-326](file://electron/services/artwork-service.ts#L262-L326)